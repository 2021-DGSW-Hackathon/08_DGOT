...
def allLedManipulate(user, flag):
    for n in led_digital_pin_list:
        GPIO.output(n, flag)
        sqlExecute("insert into led(user_name,status,led_number) values('{}',{},7)".format(
            user["name"], 1 if flag else 0))
...
def ledRoute(number, status):
    ...
    if status == "true":
        if number == "7":
            ...
            sqlExecute("insert into led(user_name,status,led_number) values('{}',1,{})".format(
                user["name"], cur_index+1))
        ...
        if number == "7":
            ...
            sqlExecute("insert into led(user_name,status,led_number) values('{}',0,{})".format(
                user["name"], cur_index+1))
...
def servoRoute(status):
  ...
  #      servo_pwm.ChangeDutyCycle(setAngle(90))
        # time.sleep(1)
        sqlExecute(
            "insert into servo(user_name,status) values('{}',1)".format(user["name"]))
        servoStatus = True
    else:
       ...
       # time.sleep(1)
        sqlExecute(
            "insert into servo(user_name,status) values('{}',0)".format(user["name"]))
        servoStatus = False
    output = {
        "led": led_digital_pin_status_list,
        "servo": servoStatus,
...
def loginRoute():
    name = request.json["name"]
    password = request.json["password"]
    sql = "select * from users where name='{}' and password='{}'".format(
        name, password)
    db_class = Database()
    print(sql)
    db_class.cursor.execute(sql)
...
def speackLed():
    ...
    for c in led_arr:
        n = int(c)
        GPIO.output(led_digital_pin_list[n-1],
                    GPIO.HIGH if status == "on" else GPIO.LOW)
...
def recordingVideo():
    ...
    camera.start_recording(
        "./static/videos/{}.h264".format(fileName))  # record video
    sleep(5)
